...
@Component({
  selector: 'app-sales-summary-cluster',
  templateUrl: './sales-summary.component.html',
  styleUrls: ['./sales-summary.component.css']
})
export class SalesSummaryClusterComponent implements OnInit, AfterViewInit {
  ...
  getProductDetails(productName, measure){
    ...
    this.productReportItems.forEach( dailyReport => {
      dailyReport['reportItems'].forEach(product => {
        if(product.title === productName && !isNaN(product.dataFormat[measure])){
               total = product.dataFormat[measure];
        }
      })
    });
